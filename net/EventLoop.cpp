...
// 每一个线程都有这么一个指针对象
    ...
    int createEventfd() {
        ...
        if (eventfd < 0) {
            LOG << "LOG_SYSERR: Failed in eventfd";
        }
        return eventfd;
    }
...
EventLoop::EventLoop()
    ...
    wakeupChannel_->setReadCallback(
            bind(&EventLoop::handleRead, this, std::placeholders::_1));//文件描述符可读后要把数据读走，否则由于是lt模式，将一直触发
...
void EventLoop::abortNotInLoopThread() {
    char msg[256];
    sprintf(msg, "EventLoop::abortNotInLoopThread - EventLoop %d was created in threadId_ = %d, current thread id =%d",
            this, threadId_, CurrentThread::tid());
}

void EventLoop::timerStart(int64_t timeIntervalMicroSeconds) { timer_->start(timeIntervalMicroSeconds); }

void EventLoop::addConnection(const timerNode &newNode, int64_t overTime) { timer_->addConnection(newNode, overTime); }

void EventLoop::removeConnection(int64_t overTimeSinceGMT, const timerNode &oldNode) {
    timer_->removeConnection(overTimeSinceGMT, oldNode);
}

void EventLoop::modifyConnection(int64_t overTimeSinceGMT, const timerNode &oldNode) {
    timer_->modifyConnection(overTimeSinceGMT, oldNode);
}